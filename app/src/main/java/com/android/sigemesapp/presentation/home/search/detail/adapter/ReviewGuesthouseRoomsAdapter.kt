...
class ReviewGuesthouseRoomsAdapter(private val listReviews: List<GuesthouseRoomReviews>) : RecyclerView.Adapter<ReviewGuesthouseRoomsAdapter.ReviewGuesthouseRoomsViewHolder>() {
    private var onItemClickCallback: OnItemClickCallback? = null

    class ReviewGuesthouseRoomsViewHolder(private val binding: ItemReviewBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(review: GuesthouseRoomReviews, callback: OnItemClickCallback?) {
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, viewType: Int): ReviewGuesthouseRoomsViewHolder {
        val binding = ItemReviewBinding.inflate(LayoutInflater.from(viewGroup.context), viewGroup, false)
        return ReviewGuesthouseRoomsViewHolder(binding)
    }

    override fun getItemCount(): Int = listReviews.size

    override fun onBindViewHolder(holder: ReviewGuesthouseRoomsViewHolder, position: Int) {
        val event = listReviews[position]
        holder.bind(event, onItemClickCallback)
    }

    interface OnItemClickCallback {
        fun onItemClicked(data: GuesthouseRoomReviews)
    }
